...
The release-please workflow runs a Linux job first: it builds the `AppImage`, `deb`, `rpm`, and `pacman` packages on `ubuntu-22.04` (a deliberate glibc floor, see [`docs/development.md`](docs/development.md#linux-packaging)) and uploads all four to the draft.
The macOS job waits for that job to finish but does not require it to succeed.
That ordering gate is advisory on purpose: the `deb`, `rpm`, and `pacman` fpm targets have never been built end to end, so a first-release fpm failure must not strand the release as a permanent draft with no DMG published. A failing Linux job still reports its real status in the workflow run. Tighten this to a hard gate once a real release has proven those targets.
...
Any signing, notarization, verification, packaged runtime, checksum, or GitHub upload failure in the macOS job leaves the release as a draft and stops before stable publication and tap publication. A Linux packaging or upload failure does not, so a release can currently publish with some or all Linux assets missing. A missing or invalid `HOMEBREW_TAP_TOKEN`, or another tap update failure, occurs after stable publication and fails the workflow without updating Homebrew.
...
To release, merge the release-please PR and require both the `release-please` workflow's Linux and macOS jobs to pass. That is a manual maintainer check before considering a release done, not something the workflow enforces today - the macOS publish job waits for the Linux job but deliberately does not gate on its success, and it becomes a real CI gate only once the hard `needs.linux.result == 'success'` check is added after a first release proves the fpm targets. The release remains a draft until the signed artifact passes verification and runtime E2E, receives a valid checksum, and uploads successfully. Do not upload a replacement DMG or Linux package, and do not update the tap, by hand.
For the interrupted `0.1.23` release only, manually dispatch the `release-please` workflow from `main` with tag `baby-menu-v0.1.23` and version `0.1.23`. This recovery path builds the immutable workflow-dispatch SHA from `refs/heads/main` without moving the existing tag, requires that commit's package version to remain `0.1.23`, requires the GitHub Release to remain a draft, and reruns the normal Linux packaging plus signed, notarized, verified macOS build and publish flow. Both jobs enforce the same recovery guards. It refuses a dispatch from another ref, any other tag or version, a checkout that differs from the dispatch commit, a mismatched package version, a missing or already-published release, or an uncertain GitHub query result. Normal releases continue building their immutable tags.
