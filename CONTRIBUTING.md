...
To release, merge the release-please PR and require both the `release-please` workflow's Linux and macOS jobs to pass. Requiring the Linux job is a manual maintainer check today, because the ordering gate above is advisory; making it a real CI gate means adding a hard `needs.linux.result == 'success'` check to the macOS job. The release remains a draft until the signed artifact passes verification and runtime E2E, receives a valid checksum, and uploads successfully. Do not upload a replacement DMG or Linux package, and do not update the tap, by hand.
