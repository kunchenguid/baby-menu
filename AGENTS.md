...
- `pnpm test:e2e` - run only `tests/e2e-*.test.ts` (these include real `acpx/runtime` coverage against `acp-mock`, bundled adapter coverage against fake local CLIs, and GitHub command-routing regression coverage).
...
- `BABY_MENU_PACKAGED_TEST_HOME=<path>` isolates packaged-app state for the packaged runtime E2E; do not use it for normal app launches.
...
- `agent-runtime.ts` - `BabyMenuAgentRuntime` wrapping `acpx/runtime`; gates every `send()` through a change session, enriches pending-session snapshots with diff-derived change labels, preserves structured turn failures, and retries once after deleting a stale persisted ACP session that reports `SESSION_RESUME_REQUIRED`.
...
- `host-command-runner.ts` - backs `context.commands.getGitHubContributionGraph()`: authorizes only the disk-inferred `github-graph.getGraph` server action, constructs the fixed GitHub contribution GraphQL argv, resolves the Settings-owned `gh` executable override only for that policy, and invokes the result directly without a shell.
...
- The GitHub contribution graph server action should use `context.commands.getGitHubContributionGraph()`; configured helper paths live in app preferences, survive upgrades, and fail closed when malformed or missing.
