...
- `preferences.ts` - stores app preferences, including the selected agent, under the active app data root and applies login-item settings only when allowed. `app.ts` allows this only for the packaged production product, matching the executable name exactly (`Baby Menu` on darwin, `baby-menu` on linux); source mode and packaged dev/test products are no-ops on every platform. On macOS and Windows this calls Electron's `setLoginItemSettings`; on Linux it calls the `linux-autostart.ts` facade instead.
