...
- `pnpm test:e2e:grok-popover` - run the unattended macOS Grok production-wiring check described in `docs/grok-quota-e2e.md`; it uses the real popover and exact consumer Grok usage source, permits only conditional official-client-owned refresh after expiry or credential rejection, and never exposes auth or raw provider data.
