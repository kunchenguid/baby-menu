...
- Public surfaces are deliberate, tested contracts: the preload bridge, `@babymenu/ui` (`src/shared/ui-exports.ts`), and `@babymenu/contracts`. External imports are limited to `react` and `@babymenu/ui` (server modules also Node builtins), plus type-only imports from `@babymenu/contracts`; relative imports of local helpers within the extension workspace are allowed.
