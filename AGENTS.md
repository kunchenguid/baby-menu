...
- `host-command-runner.ts` - backs `context.commands.execFile`: validates bare command names, argv, timeout, and output bounds, resolves the Settings-owned `gh` executable override only for the GitHub contribution graph policy, and invokes the result directly without a shell.
...
- The GitHub contribution graph server action should use `context.commands.execFile` with the fixed `gh` operation policy; configured helper paths live in app preferences, survive upgrades, and fail closed when malformed or missing.
