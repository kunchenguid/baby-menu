...
- `pnpm test:e2e:grok-popover` - run the unattended macOS Grok production-wiring check described in `docs/grok-quota-e2e.md`; it uses the real popover and exact consumer Grok usage source without refreshing credentials or exposing auth or raw provider data.
