...
| `KIMI_CODE_HOME=<dir>`            | Points the read-only Kimi quota fallback at `<dir>/credentials/kimi-code.json`; defaults to `~/.kimi-code`. Pi's supported `kimi-coding` source remains first priority.                                            |
